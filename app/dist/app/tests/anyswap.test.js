...
describe("AnySwap Client Tests", () => {
    ...
    const program = anchor.workspace.anyswap;
    const connection = provider.connection;
    const payer = provider.wallet;
    // 创建客户端
    const adminClient = new src_1.AdminClient(provider, program);
    const client = new src_1.Client(provider, program);
    // Pool 相关
    let pool;
    let poolKeypair;
    let poolAuthorityPda;
    let poolMint;
    let poolCreator;
    const poolId = new anchor_1.BN(0);
    // 费率：5/1000 = 0.5%
    const feeNumerator = new anchor_1.BN(5);
    const feeDenominator = new anchor_1.BN(1000);
    // Token 相关
    let mint0;
    let mint1;
    let mint2;
    ...
    let vault1;
    let vault2;
    let vault3; // WSOL vault
    // Payer 的 token 账户
    let payerToken0Account;
    let payerToken1Account;
    let payerToken2Account;
    const nDecimals = 9;
    it("在客户端创建 Pool", async () => {
        // 创建 pool creator（用于测试，实际使用中可以使用 provider.wallet.publicKey）
        poolCreator = web3_js_1.Keypair.generate();
        // 使用管理员客户端创建 pool
        // 注意：在测试环境中，我们需要使用 poolCreator 作为 admin
        // 但在实际使用中，可以省略 admin 参数，会使用 provider.wallet.publicKey
        const result = await adminClient.createPool(poolId, feeNumerator, feeDenominator, poolCreator.publicKey // 指定 admin
        );
        // 保存结果
        pool = result.pool;
        ...
        (0, chai_1.expect)(poolAccount.tokenCount).to.equal(0);
        (0, chai_1.expect)(poolAccount.feeNumerator.toNumber()).to.equal(5);
        (0, chai_1.expect)(poolAccount.feeDenominator.toNumber()).to.equal(1000);
    ...
    it("创建三个 token 和 vault", async () => {
        // 创建三个 mint
        mint0 = await token.createMint(connection, payer.payer, payer.publicKey, null, nDecimals);
        mint1 = await token.createMint(connection, payer.payer, payer.publicKey, null, nDecimals);
        mint2 = await token.createMint(connection, payer.payer, payer.publicKey, null, nDecimals);
        // 计算 vault PDA 地址
        vault0 = (0, src_1.getVault)(adminClient.program, pool, mint0);
        vault1 = (0, src_1.getVault)(adminClient.program, pool, mint1);
        vault2 = (0, src_1.getVault)(adminClient.program, pool, mint2);
    ...
    it("初始化三个 token，权重 20, 40, 40", async () => {
        const weight0 = new anchor_1.BN(20);
        const weight1 = new anchor_1.BN(40);
        const weight2 = new anchor_1.BN(40);
        // 添加 token0（第一个 token，pool 为空，不需要流动性）
        await adminClient.addTokenToPool(pool, mint0, weight0, [], // existingVaults
        poolCreator.publicKey // admin
        );
        // 添加 token1（需要传入现有 vault）
        await adminClient.addTokenToPool(pool, mint1, weight1, [vault0], // existingVaults
        poolCreator.publicKey // admin
        );
        // 添加 token2（需要传入现有 vaults）
        await adminClient.addTokenToPool(pool, mint2, weight2, [vault0, vault1], // existingVaults
        poolCreator.publicKey // admin
        );
        // 验证 pool 状态
        const poolAccount = await client.getPool(pool);
        (0, chai_1.expect)(poolAccount.tokenCount).to.equal(3);
        (0, chai_1.expect)(poolAccount.tokens[0].weight.toNumber()).to.equal(20);
        (0, chai_1.expect)(poolAccount.tokens[1].weight.toNumber()).to.equal(40);
        (0, chai_1.expect)(poolAccount.tokens[2].weight.toNumber()).to.equal(40);
    ...
    it("准备流动性：创建 payer 的 token 账户并铸造代币", async () => {
        // 创建 payer 的 token 账户
        payerToken0Account = await token.createAssociatedTokenAccount(connection, payer.payer, mint0, payer.publicKey);
        payerToken1Account = await token.createAssociatedTokenAccount(connection, payer.payer, mint1, payer.publicKey);
        payerToken2Account = await token.createAssociatedTokenAccount(connection, payer.payer, mint2, payer.publicKey);
        ...
        await token.mintTo(connection, payer.payer, mint0, payerToken0Account, payer.publicKey, amount0);
        await token.mintTo(connection, payer.payer, mint1, payerToken1Account, payer.publicKey, amount1);
        await token.mintTo(connection, payer.payer, mint2, payerToken2Account, payer.publicKey, amount2);
        console.log("流动性准备成功:");
    ...
    it("提供流动性到 pool", async () => {
        ...
        const baseAmount = 10000 * 10 ** nDecimals;
        const amounts = [
            new anchor_1.BN(baseAmount), // token0: 10000
            new anchor_1.BN(baseAmount * 2), // token1: 20000
            new anchor_1.BN(baseAmount * 2), // token2: 20000
        ];
        console.log("添加流动性:");
        ...
        const signature = await client.addLiquidity(pool, {
            amounts,
        ...
        (0, chai_1.expect)(vault0Balance.amount.toString()).to.equal(amounts[0].toString());
        (0, chai_1.expect)(vault1Balance.amount.toString()).to.equal(amounts[1].toString());
        (0, chai_1.expect)(vault2Balance.amount.toString()).to.equal(amounts[2].toString());
        ...
        // 验证 LP token 余额
        const userPoolAta = await token.getAssociatedTokenAddress(poolMint, payer.publicKey);
        ...
        const lpAmount = typeof lpBalance.amount === 'bigint'
            ...
            : lpBalance.amount.toNumber ? lpBalance.amount.toNumber() : Number(lpBalance.amount);
        (0, chai_1.expect)(lpAmount).to.be.greaterThan(0);
    ...
    it("测试 swap 和权重修改：用户交换 token 并验证手续费", async () => {
        ...
        const userToken1Account = await token.createAssociatedTokenAccount(connection, user, mint1, user.publicKey);
        const swapAmount1 = 10000 * 10 ** nDecimals; // 10000 token1
        await token.mintTo(connection, payer.payer, mint1, userToken1Account, payer.publicKey, swapAmount1);
        // 验证用户 token1 余额
        const userToken1BalanceBefore = await token.getAccount(connection, userToken1Account);
        (0, chai_1.expect)(userToken1BalanceBefore.amount.toString()).to.equal(swapAmount1.toString());
        ...
        // 创建用户的 token2 账户
        const userToken2Account = await token.createAssociatedTokenAccount(connection, user, mint2, user.publicKey);
        ...
        await client.swap(pool, {
            amountIn: new anchor_1.BN(swapAmount1),
        ...
        (0, chai_1.expect)(userToken2Balance.amount.toString()).to.equal(expectedAmountOut1.toString());
        ...
        (0, chai_1.expect)(vault1Increase).to.equal(swapAmount1);
        (0, chai_1.expect)(vault2Decrease).to.equal(expectedAmountOut1);
        ...
        // 4. 管理员修改权重为 40, 20, 40
        await adminClient.modifyTokenWeight(pool, mint0, new anchor_1.BN(40), poolCreator.publicKey // admin
        );
        await adminClient.modifyTokenWeight(pool, mint1, new anchor_1.BN(20), poolCreator.publicKey // admin
        );
        // 验证权重已修改
        const poolAccountAfterWeightChange = await client.getPool(pool);
        (0, chai_1.expect)(poolAccountAfterWeightChange.tokens[0].weight.toNumber()).to.equal(40);
        (0, chai_1.expect)(poolAccountAfterWeightChange.tokens[1].weight.toNumber()).to.equal(20);
        (0, chai_1.expect)(poolAccountAfterWeightChange.tokens[2].weight.toNumber()).to.equal(40);
        console.log("权重修改后:");
        ...
        // 创建用户的 token0 账户
        const userToken0Account = await token.createAssociatedTokenAccount(connection, user, mint0, user.publicKey);
        ...
        await client.swap(pool, {
            amountIn: new anchor_1.BN(swapAmount2),
            minAmountOut: new anchor_1.BN(0),
            vaultIn: vault2,
            vaultOut: vault0,
        ...
        (0, chai_1.expect)(userToken0Balance.amount.toString()).to.equal(expectedAmountOut2.toString());
        ...
        (0, chai_1.expect)(vault2Increase).to.equal(swapAmount2);
        (0, chai_1.expect)(vault0Decrease).to.equal(expectedAmountOut2);
    ...
    it("添加 token4 (WSOL/SOL) 到 pool", async () => {
        ...
        mint3 = new web3_js_1.PublicKey("So11111111111111111111111111111111111111112");
        // 计算 WSOL vault PDA 地址
        vault3 = (0, src_1.getVault)(client.program, pool, mint3);
        ...
        const weight3 = new anchor_1.BN(30);
        // 计算需要提供的 WSOL 流动性
        const requiredLiquidity = await client.calculateRequiredWSOLLiquidity(pool, weight3);
        ...
        if (requiredLiquidity.gt(new anchor_1.BN(0))) {
            ...
            await client.wrapSOL(wrapAmount, poolCreator.publicKey, // owner
        ...
        await adminClient.addTokenToPool(pool, mint3, weight3, existingVaults, // existingVaults
        poolCreator.publicKey // admin
        );
        // 验证 pool 状态
        const poolAccount = await client.getPool(pool);
        (0, chai_1.expect)(poolAccount.tokenCount).to.equal(4);
        (0, chai_1.expect)(poolAccount.tokens[0].weight.toNumber()).to.equal(40); // 已修改为 40
        (0, chai_1.expect)(poolAccount.tokens[1].weight.toNumber()).to.equal(20); // 已修改为 20
        (0, chai_1.expect)(poolAccount.tokens[2].weight.toNumber()).to.equal(40); // 保持 40
        (0, chai_1.expect)(poolAccount.tokens[3].weight.toNumber()).to.equal(30);
        (0, chai_1.expect)(poolAccount.tokens[3].mintAccount.toString()).to.equal(mint3.toString());
        (0, chai_1.expect)(poolAccount.tokens[3].vaultAccount.toString()).to.equal(vault3.toString());
